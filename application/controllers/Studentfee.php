...
class Studentfee extends Admin_Controller
{
    ...
    function addhostelfeegroup()
    {
        ...
        // Increment start_date by one month until it reaches end_date (inclusive)
        while ($start_date_obj <= $end_date_obj) {
            $month_names[] = $start_date_obj->format('F');
    ...
    function removeHostelAssign()
    {
        ...
        var_dump($this->input->post('months'));
        exit;
    ...
    {
        $student_session_id = $this->input->post('student_sesstion_id');

        $this->db->select('id');
        $this->db->from('hostel_fee_assign');
        $this->db->where("FIND_IN_SET('" . $student_session_id . "', student_session_ids)", NULL, FALSE);
        $query = $this->db->get();
        $result = $query->row();
    ...
    public function deletehosteldata()
    {
        $id = $this->input->post('id');
        $this->db->where([
            'id' => $id
