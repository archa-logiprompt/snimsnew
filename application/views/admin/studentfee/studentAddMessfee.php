                                   ...
                                   $feetype_balance = $fee_value->amount - ($fee_paid + $fee_discount);
                                       ...
                                       echo (number_format($amountfine, 2, '.', ''));
                                          ...
                                          <input type="hidden" name="balance" class="hidbalance" value="<?php echo (number_format($feetype_balance, 2, '.', '')) ?>"  />       
                                                ...
                                                <td>
                                                    <div class="btn-group pull-right"> 
                                                        <button type="button" data-student_session_id="<?php echo $fee->student_session_id; ?>" data-fine="<?php echo (number_format($fixed_fine, 2, '.', ''));  ?>" data-student_messfees_master_id="<?php echo $fee->id; ?>" data-messfeemasters_id="<?php echo $fee_value->messfeemasters_id; ?>" 
...
$fine=$fee_deposits_value->amount_fine;
                                                    ...
                                                    <tr class="white-td">
                                                        <td align="left"></td>
                                                        <td align="left"></td>
                                             <td align="left" style="color: #a9101a;"><?php if($fee_value->b_advance_amount !=''){?> Balance :   <?php echo $fee_value->b_advance_amount;  } else {?>   <?php } ?>
                                             
                                              </td>
                                                        <td align="left" style="color: #a9101a;"><?php if($fee_value->excess_amount!=0) { ?> Amount in excess : <?php echo $fee_value->excess_amount ;} ?></td>
                                                        <td align="left"></td>
                                                        ...
                                                        <td class="text text-right">
                                                            <div class="btn-group pull-right"> 
                                                                ...
                                                                <?php if ($this->rbac->hasPrivilege('collect_fees', 'can_delete')) { ?>
                                                                    <button class="btn btn-default btn-xs" data-invoiceno="<?php echo $fee_value->student_messfees_deposite_id . "/" . $fee_deposits_value->inv_no; ?>" data-main_invoice="<?php echo $fee_value->student_messfees_deposite_id ?>" data-sub_invoice="<?php echo $fee_deposits_value->inv_no ?>" data-type="<?php echo $fee_value->name ?>" data-toggle="modal" data-target="#confirm-delete" title="<?php echo $this->lang->line('revert'); ?>">
...
?></td>
                                        ...
                                        <td class="text text-right"><?php
										 ...
										 if($total_balance_amount >0)
										 {
                                            ...
                                            <div class="btn-group pull-right"> 
                                                        <button type="button" data-student_session_id="<?php echo $fee->student_session_id; ?>" data-student_messfees_master_id="<?php echo $stud_feemaster; ?>" data-headwise_fine="<?php  echo $headwise_fine?>" data-messfeemasters_id="<?php echo $feetype ?>" data-student_id="<?php echo $fee_value->student_id ?>" data-balance_amount="<?php echo $feetbalanceamount ?>" data-group="Total Fee" 
 class="btn btn-xs btn-default myCollectFeeBtn <?php echo $display_none; ?>"
            ...
            <div class="modal-body pb0">
                <div class="form-horizontal">
                    <div class="box-body">
                        ...
                        <input  type="hidden" class="form-control" id="messfeemasters_id" value="0" readonly="readonly"/>
    ...
    <div class="modal-dialog">      
        <div class="modal-content">
           ...
           <input  type="hidden" class="form-control" id="guardian_phone" value="<?php echo $student['guardian_phone'] ?>" readonly="readonly"/>
                        ...
                        <input  type="hidden" class="form-control" id="messfeemasters_id1" value="" readonly="readonly"/>
...
<div class="modal fade" id="fee_advance" role="dialog">
    <div class="modal-dialog">      
        ...
        <h4 class="modal-title title text-center fees_title"> Fees received in advance</h4>
           ...
           <input type="hidden" class="form-control" id="stud_name" value="<?php echo $student["firstname"] . " " . $student["lastname"]; ?>" />
            <input type="hidden" class="form-control" name="messfee_session_id" id="messfee_session_id"value="<?php echo $messfeesession_id ?>"  />
...
<script type="text/javascript">
    $(document).on('click', '.save_button', function (e) {
        ...
        var messfeemasters_id = $('#messfeemasters_id').val();
        ...
        $.ajax({
            ...
            type: 'post',
            data: {date: date,group:group,feename:type, type: feetype, amount: amount, amount_fine: amount_fine, description: description, student_messfees_master_id: student_messfees_master_id, messfeemasters_id: messfeemasters_id, payment_mode: payment_mode,stud_name:stud_name, guardian_phone: guardian_phone, guardian_email: guardian_email,ad_invo:ad_invo,student_id:student_id},
            dataType: 'json',
            success: function (response) {
                // console.log(response);
                $this.button('reset');
...
<script type="text/javascript">
    ...
    $(document).on('click', '.save_button2', function (e) {
        ...
        var messfeemasters_id = $('#messfeemasters_id1').val();
		var stud_name=$('#stud_name').val();
		
		if(messfeemasters_id.length >1)
		{
		 var  feegroupnew = messfeemasters_id.split(','); 	
			
		}
		
		else
		{
			feegroupnew = messfeemasters_id;
       ...
       // var student_fees_discount_id = $('#discount_group').val();
        ...
        $.ajax({
            ...
            type: 'post',
            data: {date: date,cal_amount:cal_amount, type: feetype, amount:amount, amount_fine: amount_fine, description: description, student_messfees_master_id: student_messfees_master_id, messfeemasters_id: feegroupnew, payment_mode: payment_mode, guardian_phone: guardian_phone, guardian_email: guardian_email,balance:balancenew,stud_name:stud_name,invo:invo },
            dataType: 'json',
            success: function (response) {
                // console.log(response);
                $this.button('reset');
...
<script type="text/javascript">
    ...
    $("#myFeesModal").on('shown.bs.modal', function (e) {
		...
		var fine=data.fine;
        var messfeemasters_id = data.messfeemasters_id;
        ...
        $('#messfeemasters_id').val(messfeemasters_id);
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            data: {'messfeemasters_id': messfeemasters_id,
...
$(".t_fee_type").select2({
  tags: true,
    ...
    $("#myFeesModal2").on('shown.bs.modal', function (e) {
		...
		if(messfeemasters_id.length >1)
		{
		var messfeemasters_idnew = messfeemasters_id.split(',');	
		}
		else
		{
			var array_id=[];
			array_id.push(messfeemasters_id);
			messfeemasters_idnew = array_id;
	...
	/*	$('#totalbalance_amount').val(balance);*/
		...
		*/
		   console.log(messfeemasters_idnew); 
        ...
        $.ajax({
            ...
            data: {
			    'messfeemasters_id': messfeemasters_idnew,
                'student_messfees_master_id': student_messfees_master_id,
                'student_session_id': student_session_id,
            ...
            success: function (data) {
				...
				var weeks=Math.round(days/7);		
					...
					if(obj.finetype=='Monthly')
					 ...
					 t_feetype +="<option data-fine="+fixed_fine+" data-amount="+obj.amount+" value="+obj.messfeemasters_id+ "> "+obj.type+" ["+obj.amount+"]</option>";	
					}
					else 
					{
					...
					//discount=Number(discount)+Number(t.amount_discount); 
					 ...
					 if(balance_amount!==0)
					 {
						...
						t_feetype +="<option data-fine="+datafine+" data-amount="+balance_amount+" value="+obj.messfeemasters_id+ " > "+obj.type+ " ["+ balance_amount +" ]</option>";	
						}
    ...
    $('#messfeemasters_id1').val(samplesSelected);
...
<script type="text/javascript">
	 ...
	 $(document).on('click', '.btn_feeadvance', function (e) {
		    ...
		    var stud_name=$('#stud_name').val();
			var messfee_session_id=$('#messfee_session_id').val();
			var student_session_id='<?php echo $student['student_session_id'] ?>';
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                data: {'ad_amount': ad_amount, 'ad_date': ad_date,'ad_note':ad_note,'payment_mode_fee':payment_mode,'student_id':student_id,'stud_name':stud_name,ad_invo:ad_invo,student_session_id:student_session_id,messfee_session_id:messfee_session_id},
                success: function (data) {
