    ...
    $(function () {
        ...
        $(document).on("click", "#btnAdd", function () {
            ...
            $("#TextBoxContainer").append(div);
                 	 $('.selectpickerteacher').selectpicker('render'); 
...
foreach ($subjectlist as $subject) {
    ?>
            row += '<option value="<?php echo $subject['id'] ?>"><?php echo $subject['name'] . " (" . $subject['code'] . ")" ?></option>';
    <?php
    $count++;
}
?>
	...
	//row += '</div>;'
     ...
     row += '<div class="col-md-4">';
   row += '<select multiple="multiple"  id="teacher_id_' + value + '" name="teacher_id_' + value + '[]" no="' + value + '" class="form-control selectpickerteacher" >';
   row += '<option value=""><?php echo $this->lang->line('select'); ?></option>';
<?php
...
foreach ($subjectlist as $subject) {
    ?>
           var selected = "";
            ...
            row += '<option value="<?php echo $subject['id'] ?>" ' + selected + '><?php echo $subject['name'] . " (" . $subject['code'] . ")" ?></option>';
