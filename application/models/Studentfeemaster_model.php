...
class Studentfeemaster_model extends CI_Model
{
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null)
    {
        ...
        $sql .= " ORDER BY `students`.`firstname`";
    ...
    public function addHostelFee($data)
    {
        ...
        foreach ($studentfee_existing as $key => $studentfee) {
            ...
            if ($feegroup_exist->hostel_room_fee_id != null && $feegroup_exist->hostel_room_fee_id != $data['hostel_roomid']) {
                $this->db->where([
                    'id' => $fee_session_groups_exist->fee_groups_id
                ])->delete('fee_groups');
                $this->db->where([
        ...
        if (!in_array($data['hostel_roomid'], $hostelroom_exist_ids)) {

            $feegroupname = $data['feegroupname'];
            $roomname = explode('_', $feegroupname)[1];
            $admin = $this->session->userdata('admin');
            $finyear = substr($admin['financial_year'], 2, 4);
            $hostelid = $data['hostel_roomid'];
            $months = $data['months'];


            $this->db->where('id', $data['student_id'])->update('students', [
            ...
            $fee_groups_feetype_hostel_details = $this->db
                ->join('fee_groups', 'fee_groups_feetype.fee_groups_id=fee_groups.id')
                ->where([
                    'hostel_room_id' => $hostelid
                ])->get('fee_groups_feetype')->row();


            if (count($months) == 12) {
                $month_count = count($months);
                $feegroupname = "Hostel Fee " . $roomname . " - Yearly (" . $months[0] . " to " . $months[$month_count - 1] . ")";
                $feegroupdata = [
                    ...
                    'year' => $finyear,
                    'is_active' => 'no',
                    'hostel_room_fee_id' => $hostelid
                ];

                $this->db->insert('fee_groups', $feegroupdata);
                $feegroup_id = $this->db->insert_id();


                $fee_session_groups_data = [
                    'fee_groups_id' => $feegroup_id,
                    'session_id' => 19,
                    'is_active' => 'no',
                ];

                $this->db->insert('fee_session_groups', $fee_session_groups_data);

                $fee_session_groups_id = $this->db->insert_id();


                $fee_groups_feetype_data = [

                    'centre_id' => $admin['centre_id'],
                    'fee_session_group_id' => $fee_session_groups_id,
                    'fee_groups_id' => $feegroup_id,
                    'feetype_id' => $fee_groups_feetype_hostel_details->feetype_id,
                    'session_id' => 19,
                    'due_date' => '',
                    'amount' => ($fee_groups_feetype_hostel_details->amount) * 12,
                    'addfine' => '0',
                    'finetype' => '0',
                    'amounttype' => '0',
                    'fixedamount' => '0',
                ...
                $this->db->insert('fee_groups_feetype', $fee_groups_feetype_data);

                $student_fees_master_data = array(
                    'centre_id' => $admin['centre_id'],
                    'student_session_id' => $data['student_session_id'],
                    'fee_session_group_id' => $fee_session_groups_id,
                ...
                $this->db->where('student_session_id', $student_fees_master_data['student_session_id']);
                $this->db->where('fee_session_group_id', $student_fees_master_data['fee_session_group_id']);
                $q = $this->db->get('student_fees_master');

                if ($q->num_rows() > 0) {
                    $q->row()->id;
                } else {
                    $this->db->insert('student_fees_master', $student_fees_master_data);
                    $this->db->insert_id();
                }
            } else {
                $month_count = count($months);

                $month = ucfirst($months);
                $feegroupname = "Hostel Fee " . $roomname . " - " . $months[0] . " to " . $months[$month_count - 1];
                $feegroupdata = [
                    'centre_id' => $admin['centre_id'],
                    'name' => $feegroupname,
                    'class_id' => 0,
                ...
                $fee_groups_feetype_data = [
                    ...
                    'session_id' => 19,
                    'due_date' => '',
                    'amount' => $fee_groups_feetype_hostel_details->amount * $month_count,
                    'addfine' => '0',
                    'finetype' => '0',
                    'amounttype' => '0',
                    'fixedamount' => '0',
    ...
    public function addHostelInFeeMaster($data)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $this->db->select('fee_groups.hostel_room_id');
            ...
            $result = $this->db->get('student_fees_master')->row()->hostel_room_id;
    ...
    public function addTransportInFeeMaster($data)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $this->db->select('fee_groups.vehroute_id');
            $this->db->join('fee_session_groups', 'fee_session_groups.id=student_fees_master.fee_session_group_id');
            $this->db->join('fee_groups', 'fee_session_groups.fee_groups_id=fee_groups.id');
            $this->db->where('student_session_id', $data['student_session_id']);

            $result = $this->db->get('student_fees_master')->row()->vehroute_id;
    ...
    public function fee_deposit($data, $send_to, $student_fees_discount_id, $invoice)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            $this->db->where(array('invoice.year' => $admin['financial_year'], 'invoice.centre_id' => $admin['centre_id'], 'invoice.invoice_no' => $invoice_id));
 ...
 */
    ...
    public function collection_report($start_date, $end_date, $collected_by = null)
    {
        $admin = $this->session->userdata('admin');
        $centre_id = $admin['centre_id'];
        ...
        $this->db->join('sections', 'sections.id= student_session.section_id');
        $this->db->join('students', 'students.id=student_session.student_id');
        // $this->db->join('fees_advance', 'students.id=fees_advance.student_id', 'left'); 

        if ($collected_by != 'null' && $collected_by != null) {
            $this->db->where('student_fees_deposite.collected_by', $collected_by);
        }
        $this->db->where('students.centre_id', $centre_id);
        $this->db->order_by('student_fees_deposite.id');
        $this->db->group_by('student_fees_deposite.id');
        ...
        // $student_id = [];
        // $student_id_repeated = [];
        // $student_id_only_once = [];
        // $student_advance_amount = [];

        // foreach($result_value as $result){

        //     $student_id[]=$result->student_id;
        //     $student_advance_amount[]=$result->advance_amount_detail;

        //     if($this->isItemRepeated($student_id,$result->student_id)){ 
        //         $student_id_repeated[] = $result->student_id;
        //         if(!($this->isItemRepeated($student_id_repeated,$result->student_id))){
        //             $student_id_only_once[] = $result->student_id;
        //             $studentid = $result->student_id;

        //             $advance_amount = array_filter($result_value,function($resultcallback) use($studentid){
        //                 return $resultcallback->student_id==$studentid; 
        //             });

        //             var_dump($advance_amount);exit;
        //         }
        ...
        if (!empty($result_value)) {
            ...
            foreach ($result_value as $key => $value) {
                $return = $this->findObjectById($value, $st_date, $ed_date);
                $advance_amount_json = $this->db->select('fees_advance.amount_detail')->where('student_id', $value->student_id)->get('fees_advance')->result();
                $advance = [];
                foreach ($advance_amount_json as $jsonval) {
                    $advance[] = $this->findObjectById($jsonval, $st_date, $ed_date)[0];
                }
                $advance_sum[$value->student_id] = array_sum(array_column($advance, 'amount'));


                $student_ids = array_column($result_value, 'student_id');
                $student_id_count = array_count_values($student_ids);
                ...
                if (!empty($return)) {
                    foreach ($return as $r_key => $r_value) {
                        ...
                        // if($student_id_count[$value->student_id]>1){

                        if (!in_array($value->student_id, array_column($return_array, 'student_id'))) {
                            if ($advance_sum[$value->student_id] == '0') {

                                $a['advance'] = '';
                            } else {

                                $a['advance'] = $advance_sum[$value->student_id];
                            }
                        ...
                        // TODO Need To Fetch Discount here
    ...
    function isItemRepeated($array, $item)
    {
        $countValues = array_count_values($array);
        return isset($countValues[$item]) && $countValues[$item] > 1;
    }
    ...
    public function get_feeadvanceReport($date_from, $date_to, $payment_mode, $student_id = null)
    {
        $admin = $this->session->userdata('admin');
        $centre_id = $admin['centre_id'];
        $this->db->select('students.admission_no,students.firstname,students.lastname,fees_advance.*')->from('fees_advance');
        $this->db->join('students', 'fees_advance.student_id=students.id');
        $this->db->where('students.centre_id', $centre_id);
        if ($student_id != null) {

            $this->db->where_not_in('fees_advance.student_id', $student_id);
    ...
    public function getFeeByInvoice($invoice_id, $sub_invoice_id)
    {
        ...
        // $sub = substr($admin['financial_year'], 0, 2);
        // var_dump($sub);exit;
        $this->db->select('student_fee_deposit_id')->where('id', $invoice_id)->where("SUBSTRING(year, 1, 2) =", $sub_invoice_id);
    ...
    {
        $this->db->select('student_fees_deposite.amount_detail,student_fees_deposite.id,feetype.type')->from('student_fees_master');
...
');
    ...
    public function advance_balance_add($data)
    {

        $existing = $this->db->select('*')->where('student_id', $data['student_id'])->get('advance_balance')->row();

        $current_class_section = $this->db->select('*')->where('student_id', $data['student_id'])->get('student_session')->row();

        $data['changes']['class'] = $current_class_section->class_id;
        $data['changes']['section'] = $current_class_section->section_id;

        if (empty($existing)) {

            $changes[0] = $data['changes'];
            $data['changes'] = json_encode($changes);

            $this->db->insert('advance_balance', $data);
        } else {
            $changes = json_decode($existing->changes);
            // $count = count($changes);
            array_push($changes, $data['changes']);
            ...
            $this->db->where('student_id', $data['student_id'])->update('advance_balance', [
                'amount' => $existing->amount + $data['amount'],
                'changes' => json_encode($changes),
    ...
    public function excess_balance_add($data)
    {


        $existing = $this->db->select('*')->where('student_id', $data['student_id'])->get('excess_balance')->row();
        $current_class_section = $this->db->select('*')->where('student_id', $data['student_id'])->get('student_session')->row();
        ...
        if (empty($existing)) {
            ...
            $this->db->insert('excess_balance', $data);
        } else {
            $changes = json_decode($existing->changes);
            // $count = count($changes);
            array_push($changes, $data['changes']);
            ...
            $this->db->where('student_id', $data['student_id'])->update('excess_balance', [
                'amount' => $existing->amount + $data['amount'],
                'changes' => json_encode($changes),
            ]);
    ...
    public function debit_from_advance($data)
    {
    ...
    public function debit_from_excess($data)
    {
