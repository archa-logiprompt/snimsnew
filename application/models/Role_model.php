...
// # Updating data
    ...
    public function count_roles($id) {
     ...
     $admin=$this->session->userdata('admin');
        $query = $this->db->select("*")->join("staff", "staff.id = staff_roles.staff_id")->where("staff_roles.role_id", $id)->where("staff.is_active", 1)->where("staff.centre_id",$admin['centre_id'])->get("staff_roles");

        return $query->num_rows();
    }
	
	
	public function get_teacher_role_id()
	{
		  $admin=$this->session->userdata('admin');
		  $this->db->select('id')->from('roles')->where(array('name'=>'Teacher','centre_id'=>$admin['centre_id']));
