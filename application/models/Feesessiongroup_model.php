...
class Feesessiongroup_model extends CI_Model {
    ...
    public function getHostelFeesByGroup($id = null) {
        ...
        //$this->db->where('fee_session_groups.session_id', $this->current_session);
        $this->db->where('fee_groups.is_system', 0); 
        $this->db->where('fee_groups.hostel_room_id', $id);
        $query = $this->db->get();
        ...
        foreach ($result as $key => $value) {
            $value->feetypes = $this->getfeeHostelTypeByGroup($value->fee_groups_id, $value->id);
    ...
    public function getfeeHostelTypeByGroup($id = null, $fee_session_group_id) {
    ...
    public function getfeeTypeByGroup($id = null, $fee_session_group_id) {
        ...
        $this->db->where('fee_groups_feetype.fee_groups_id', $id);
        $this->db->where('fee_groups_feetype.fee_session_group_id', $fee_session_group_id);
        $this->db->where('fee_groups_feetype.is_visible', 1);
        $this->db->order_by('fee_groups_feetype.id', 'asc');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function valid_check_exists($str) {
        ...
        if ($this->check_data_exists($fee_groups_id, $feetype_id, $id)) {
            $this->form_validation->set_message('check_exists', 'Fee Group combination already exists');
            return FALSE;
        } else {
            return TRUE;
        }
	...
	public function add_messfee($data) {
      
	  if($data['id']!='')
		{
			$this->db->where('id',$data['id']);
			$this->db->update('mess_fee',$data);
			
		}
		else{
			$data['id']=1;
		$this->db->insert('mess_fee',$data);
	...
	public function get_messfee() {
		
	$this->db->select()->from('mess_fee');
