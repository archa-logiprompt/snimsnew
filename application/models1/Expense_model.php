...
class Expense_model extends CI_Model {
    ...
    public function search($text = null, $start_date = null, $end_date = null) {
        ...
        if (!empty($text)) {
            ...
            $this->db->where('expenses.centre_id',$centre_id);
    ...
    public function getTotalExpenseBydate($date) {
		$admin=$this->session->userdata('admin');
        $query = 'SELECT sum(amount) as `amount` FROM `expenses` where date=' . $this->db->escape($date).'and centre_id='.$admin['centre_id'];
