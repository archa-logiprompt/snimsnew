...
class Teachersubject_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT teacher_subjects.*,exam_schedules.date_of_exam,exam_schedules.start_to,exam_schedules.end_from,exam_schedules.room_no,exam_schedules.full_marks,exam_schedules.passing_marks,exam_schedules.topics,exam_schedules.teacher as scheduleteacher,subjects.name,subjects.type FROM `teacher_subjects` LEFT JOIN `exam_schedules` ON exam_schedules.teacher_subject_id=teacher_subjects.id AND exam_schedules.exam_id = " . $this->db->escape($exam_id) . "  INNER JOIN subjects
    ...
    public function getSubjectByClsandSectionNew($class_id, $section_id)
    {
        ...
        $sql = "SELECT teacher_subjects.*, subjects.name,subjects.type,subjects.theory,subjects.practical FROM `teacher_subjects` INNER JOIN subjects ON teacher_subjects.subject_id = subjects.id INNER JOIN class_sections ON teacher_subjects.class_section_id = class_sections.id WHERE class_sections.class_id =" . $this->db->escape($class_id) . " and class_sections.section_id=" . $this->db->escape($section_id)." GROUP BY teacher_subjects.subject_id";
