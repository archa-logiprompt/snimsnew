...
class Hr_model extends CI_Model {
    ...
    public function revertPayslipStatus($payslipid) {
     ...
     function payrollYearCount() {
        ...
        $query = $this->db->select("distinct(year) as year")->order_by('year')->get("feeyear");

        return $query->result_array();
    }

function gethrReport($month, $year, $role) {
		
        $sql = "SELECT DISTINCT staff.id,staff.employee_id as `employ_code`,staff.name as `employ_name`,roles.name as `user_type`,staff_designation.designation as `designation`,department.department_name as `department` FROM staff INNER JOIN staff_designation on staff.designation = staff_designation.id INNER JOIN department on staff.department = department.id INNER JOIN staff_roles on staff_roles.staff_id = staff.id INNER JOIN roles on staff_roles.role_id = roles.id INNER JOIN staff_leave_details on staff.id=staff_leave_details.staff_id  WHERE roles.id=".$this->db->escape($role);
...
function getworking(){
	$sql="SELECT * FROM staff_leave_request";
	$query=$this->db->query($sql);
	return $query->result_array();
} 
