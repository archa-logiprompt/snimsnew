  ...
  public function clinicalrotation()
    {
        ...
        if ($this->form_validation->run() == false) {
           ...
           $data['groups'] = $this->db->from('clinical_group')
            ...
            ->where($where_array)
            // ->order_by('clinical_groupname.group_name')
                ->group_by('assign_ward.id','clinical_groupname.group_name','assign_ward.datefrom','assign_ward.dateto')           
            // ->group_by('clinical_group.group_id ')           
            ->distinct()
            ->get()
            ->result_array();
            ...
            foreach ($groups as $item) {
                $dateRange = date('d/m/y', strtotime($item['datefrom'])) . ' - ' . date('d/m/y', strtotime($item['dateto']));
                if (!in_array($dateRange, $dateRanges)) {
                    $dateRanges[] = $dateRange;
                }
            }
            // usort($dateRanges, function($a, $b) {
            //     $dateA = strtotime(explode(' - ', $a)[0]);
            //     $dateB = strtotime(explode(' - ', $b)[0]);
            //     return $dateA - $dateB;
            // });

            $data['dates'] = $dateRanges;
            ...
            foreach ($groups as $item) {
                ...
                // If the group doesn't exist in $groupedData, create it
                if (!array_key_exists($groupName, $groupedData)) {
            ...
            foreach ($groupedData as $groupName => &$group) {
                usort($group, function($a, $b) {
                    $datefrom_a = date('d/m/y', strtotime($a['datefrom']));
                    $datefrom_b = date('d/m/y', strtotime($b['datefrom']));
                    $dateto_a = date('d/m/y', strtotime($a['dateto']));
                    $dateto_b = date('d/m/y', strtotime($b['dateto']));
                    return strtotime($datefrom_a) <=> strtotime($datefrom_b) ?: strtotime($dateto_a) <=> strtotime($dateto_b);
                });
            }
