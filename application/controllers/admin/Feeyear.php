...
class Feeyear extends Admin_Controller
{
    ...
    function index()
    {
        ...
        // $this->form_validation->set_rules('year','Year','required|is_unique[feeyear.year]');
        $this->form_validation->set_rules('year', 'Year', 'callback_unique_feeyear');
    ...
    public function unique_feeyear($str)
    {

        $admin = $this->session->userdata('admin');
        $centre_id = $admin['centre_id'];

        $custom_validation_result = $this->db->where([
            'year' => $str,
            'centre_id' => $centre_id,
        ])->get('feeyear')->row();

        if ($custom_validation_result) {
            $this->form_validation->set_message('unique_feeyear', 'The {field} field must be unique.');
            return false;
        }
        return true;
    }
