  ...
  margin-left: 4cm;
    ...
    width: 100px; padding: 3px; border: 3px solid #d2d6de;" class="profile-user-img img-responsive img-circle" src="<?php if(!empty($student['image'])){ echo base_url() .$student['image']; }else { echo base_url()."uploads/student_images/no_image.png"; } ?>" alt="User profile picture">
		  ...
		  $y=date('Y',$year);
           ...
           <center><label> <b> GENERAL EDUCATION </b>  </label></center>
      ...
      <tr>
        ...
        <th>PERCENTAGE OF MARKS</th>
         ...
         <td></td>
           ...
           <center><label><b> BASIC B.Sc NURSING EXAMINATION DETAILS </b> </label></center>
            <table style="width: 100%;" border="1">    
           <thead>
      <tr>
        <th rowspan="2">NAME OF</th>
        <th colspan="6">MONTH & YEAR OF APPEARANCE WITH REMARKS (PASSED / FAILED)</th>
        <th rowspan="2">AGGREGATE MARKS WITH PERCENTAGE
(To be entered only if passed)
</th>  
      </tr>
      ...
      <th style="text-align:center;"> VI</th>
		 ...
		 foreach( $appearence as $key=>$appear)
		 {
			 $total_mark=0;
         ...
         <tr>
         <td><?php echo $appear->section ?></td>
        
         <?php 
		 $c=count($appear->appear);
		if(!empty($appear->appear))
		{
		  ...
		  foreach($appear->appear[0]['chance'] as $fmark)
		  {
			$full_mark=$full_mark+ $fmark['full_marks']; 
		  }
		  
		 
		  
		  
		 foreach($appear->appear as $val){
			
			 $result="Passed";
			 
			
			 foreach($val['chance'] as $chance)
			 {
				  
				  
				 if($result=="Passed")
				{
				if($chance['get_marks'] < $chance['passing_marks'])
				{
				...
				if($chance['get_marks'] >= $chance['passing_marks'])
				{
				$aggregate_mark = (int) $aggregate_mark + (int) $chance['get_marks'];
			 ...
			 $per= $aggregate_mark*100 / $full_mark;
			 $total_mark=$aggregate_mark .'/'.$full_mark .' ( '.number_format($per, 2, '.', '') .' % )  ';
			
			 
			  ?>
         <td style="text-align:center;"> <?php echo $val['date']; ?> <?php echo $result; ?>  </td>
         
         <?php } 
		 
		  $agg_fullmark= $agg_fullmark +$full_mark;
		  $grand_total =  $grand_total+$aggregate_mark;
		 }
         ...
         <td style="text-align:center;"> <?php if($result=="Passed") { 
		  //$per= $aggregate_mark*100 / $full_mark;
		 
echo $total_mark; //$aggregate_mark .'/'.$full_mark .' ( '.number_format($per, 2, '.', '') .' % )  '; 
         ...
         <td style="text-align:center;" colspan="7"> GRAND TOTAL</td>
         <td style="text-align:center;" >  <?php   
		 
		 if($result=="Passed"){
		 
		 
		 echo $grand_total.  '/'.  $agg_fullmark ; }?></td>
         ...
         <td style="text-align:center;" colspan="7"> PERCENTAGE</td>
         <td style="text-align:center;"  > <?php 
		if($result== "Passed"){
		
		   $total_per= $grand_total *100/$agg_fullmark ;
		   echo number_format($total_per, 2, '.', '');
         ...
         <td style="text-align:center;" colspan="7"> GRADE</td>
         <td></td>
      ...
      <tr>
        ...
        <th style="text-align:center;">SICK LEAVE </th>
         ...
         <tr>
          <td style="text-align:center;font-weight: bold; ">OVER ALL
GRADE
</td>
          <td></td>
