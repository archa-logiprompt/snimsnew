...
class Feesforward extends Admin_Controller {
            ...
            if ($action == 'search') {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    foreach ($counter as $count_key => $count_value) {
						...
						$student_array['student_id']=$this->input->post('student_id['.$count_value.']');
    ...
    public function findPreviousBalanceFees($session_id, $class_id, $section_id, $current_session) {
        ...
        if (!empty($studentlist)) {
            ...
            foreach ($studentlist as $student_list_key => $student_list_value) {
				...
				$obj->student_id=$student_list_value->student_id;
	...
	function messfeeforward() {
        if (!$this->rbac->hasPrivilege('fees_carry_forward', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'mess fee');
        $this->session->set_userdata('sub_menu', 'mess fee/fee cary forward');
        $data['title'] = 'Add Feesforward';
        $data['title_list'] = 'Recent FeeType';
        $class = $this->class_model->get();
        $data['classlist'] = $class;
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if ($action == 'search') {
                ...
                $this->form_validation->set_rules('section_id', 'Section', 'required');
                if ($this->form_validation->run() == TRUE) {
                    $student_Array = json_decode($this->findPreviousBalanceMessFees($pre_session->id, $class_id, $section_id, $current_session));
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    foreach ($counter as $count_key => $count_value) {
						...
						$student_array['student_id']=$this->input->post('student_id['.$count_value.']');
                        $student_array['is_system'] = 1;
                        $student_array['mess_fee_session_id'] = 0;
                        $student_data[] = $student_array;
                    }

                    $student_due_fee = $this->studentfeemaster_model->addPreviousMessBal($student_data, $due_date);

                    $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">
                        Fee alloted successfully</div>');
                    redirect('admin/feesforward/messfeeforward');
                }
            }
        }
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/feesforward/messfeeforward', $data);
        $this->load->view('layout/footer', $data);
    }
	 ...
	 public function findPreviousBalanceMessFees($session_id, $class_id, $section_id, $current_session) {
        ...
        if (!empty($studentlist)) {
            ...
            $student_session_array = "(" . implode(",", $student_comma_seprate) . ")";
            $record_exists = $this->studentfeemaster_model->getMessBalanceMasterRecord($this->balance_group, $student_session_array);
            ...
            if (!empty($record_exists)) {
                ...
                foreach ($student_Array as $stkey => $eachstudent) {
                    ...
                    if ($eachstudent->student_previous_session_id != "") {
                        ...
                        $student_total_fees = $this->studentfeemaster_model->getPreviousStudentMessFees($eachstudent->student_previous_session_id);
                    }
