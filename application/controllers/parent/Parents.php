...
class Parents extends Parent_Controller
{
    ...
    function gettimetable($id = NULL)
    {
        ...
        $section_id = $student['section_id'];
        $data['title'] = 'Student Details';
        $result_subjects = $this->teachersubject_model->getSubjectByClsandSectionnewplan($class_id, $section_id, 'Theory');
    ...
    function getsubject($id = NULL)
    {
        ...
        $section_id = $student['section_id'];
        $data['title'] = 'Student Details';
        $subject_list = $this->teachersubject_model->getSubjectByClsandSectionnewval($class_id, $section_id, 'Theory');
    ...
    public function assignment()
    {
        ...
        if ($student_id == 0) {
            $student_id = $_SESSION['parent_childs'][0]['student_id'];
    ...
    function getfeedback($id = null)
    {
        ...
        $parent_details = $this->student_model->getParentDetails($student_id);
        ...
        if (isset($_POST['submit'])) {
            ...
            redirect("parent/parents/getfeedback/$id", 'refresh');

        } else {
            $this->load->view('layout/parent/header');
            $this->load->view('parent/student/feedback', $data);
            $this->load->view('layout/parent/footer');
        }
