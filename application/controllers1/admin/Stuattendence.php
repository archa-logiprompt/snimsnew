        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($students as $index => &$student) {
                ...
                foreach ($dates as $date) {
                    $student['attendancedata'][$i] = $this->db->select('attendencetype')->from('student_period_attendance')->where('subject_id', $subject)->where('student_id', $student_id)->where('class_id', $class)->where('section_id', $section)->where('date', date('d/m/Y', strtotime($date)))->get()->row()->attendencetype;
