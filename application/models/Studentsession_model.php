...
class Studentsession_model extends CI_Model {
    ...
    public function getTotalStudentBySession() {
        ...
        $query = "SELECT count(*) as `total_student` FROM `student_session` INNER JOIN students on students.id=student_session.student_id where student_session.session_id=" . $this->db->escape($this->current_session) . " and students.is_active = 'yes' and students.centre_id=".$admin['centre_id'];
