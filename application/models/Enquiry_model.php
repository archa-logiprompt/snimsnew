...
class enquiry_model extends CI_Model {
    ...
    if(!empty($id) and !empty($status)){
      ...
      where('enquiry.status', $status)->where('enquiry.centre_id',$centre_id)->order_by("enquiry.id","desc")->
      get("enquiry");
    ...
    public function searchEnquiry($source, $status='active', $date_from, $date_to) {
       ...
       $centre_id=$admin['centre_id'];
        ...
        $query = $this->db->select('enquiry.*,classes.class as classname')->join("classes", "classes.id = enquiry.class", "left")->where('enquiry.centre_id',$centre_id)->get("enquiry");
        return $query->result_array();
    }
