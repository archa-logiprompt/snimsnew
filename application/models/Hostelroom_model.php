...
class Hostelroom_model extends CI_Model
{
    ...
    public function add($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $insert_array = array(
                ...
                'feetype_id' => $feetype,
                'amount' => $this->input->post('total_amount_final'),
    ...
    public function searchHostelDetails($section_id, $class_id, $hostel_name = "")
    {
        ...
        $query = $this->db->select('students.firstname,students.id as sid, students.admission_no,,students.guardian_phone,classes.class,sections.section,students.lastname,students.mobileno,hostel_rooms.*,hostel.hostel_name,room_types.room_type,sessions.session')
            ...
            ->where('students.centre_id', $admin['centre_id'])->where($condition)
    ...
    public function amountChange($data)
    {

        $fee_groups_id = $this->db->select('id')->where('hostel_room_fee_id', $data['id'])->get('fee_groups')->result_array();

        $fee_groups_session_id = array();
        $fee_groups_session_id = array();
        foreach ($fee_groups_id as $group_id) {
            // Use $group_id['id'] directly in the where clause
            $session_ids = $this->db->select('id')->where('fee_groups_id', $group_id['id'])->get('fee_session_groups')->result_array();
            // Merge the session IDs into $fee_groups_session_id
            $fee_groups_session_id = array_merge($fee_groups_session_id, $session_ids);
        }
        $message = '';
        foreach ($fee_groups_session_id as $fee_groups_feetype_id) {
            $paid = $this->db->select('amount_detail')->join('fee_groups_feetype', "fee_groups_feetype.id=student_fees_deposite.fee_groups_feetype_id")->where('fee_session_group_id', $fee_groups_feetype_id['id'])->get('student_fees_deposite')->result_array();


            if ($paid) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Hostel Room was not updated because fee has already been collected</div>');
                redirect('admin/hostelroom/index');
            }
        }

        foreach ($fee_groups_session_id as $session_id) {
            $this->db->where('fee_session_group_id', $session_id['id'])->update('fee_groups_feetype', ['amount' => $data['cost_per_bed']]);
