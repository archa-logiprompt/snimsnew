                    ...
                    <div class="invoice">
                        ...
                        <div class="row">                           
                            <div class="col-xs-6">
                                <br/>
                                <address>
                                    <strong><?php echo $feearray[0]->firstname . " " . $feearray[0]->lastname; ?></strong><br><?php echo $this->lang->line('father_name'); ?>: <?php echo $feearray[0]->father_name; ?><br><?php echo $this->lang->line('class'); ?>: <?php echo $feearray[0]->class . " (" . $feearray[0]->section . ")"; ?>
                              </address>
                           	</div>
                        ...
                        if (empty($feeList->amount_detail) && $current_date > $due_date) {
							 
							if($feeList->finetype=='Monthly')
							{ 
								$i=0;
								while($i<$months){
									if($feeList->amounttype =='Fixed Amount')
									{		
										$fixed_fine = $fixed_fine+$feeList->fixedamount;
									} 
									else if($feeList->amounttype =='Percentage') 
									{
										$per=($feeList->percentage/100)*$feeList->amount;
							...
							else if($feeList->finetype=='Weekly')
							{
					        	$i=0;
								while($i<$week){
									if($feeList->amounttype =='Fixed Amount')
							...
							else if($feeList->finetype=='Daily')
							{
								$i=0;
								while($i<$days){
									if($feeList->amounttype =='Fixed Amount')
                        ...
                        <tr class="dark-gray">
                            ...
                            <td class="">
							<?php if ($feeList->due_date == "0000-00-00") 
							{
                            }
							else
							{
						...
						<?php echo ($currency_symbol . number_format($fee_paid,2,'.',''));?>
                        	</td>
                            <td class="text text-right"><?php
                                                        echo ($currency_symbol . number_format($fixed_fine, 2, '.', ''));
                                                        ?></td>
                                                    <td class="text text-right"><?php
