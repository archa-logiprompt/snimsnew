...
*/
   ...
   /* function getByFeecategory() {
	...
	function mess_income_head() {
        ...
        $this->session->set_userdata('top_menu', 'mess fee');
        $this->session->set_userdata('sub_menu', 'mess fee/mess income head');
        $data['title'] = 'Add Income';
        $data['title_list'] = 'Recent Incomes';
        ...
        if ($this->form_validation->run() == FALSE) {
		  ...
		  $data = array(
                ...
                'category'=>$this->input->post('incomehead'),
                'description' => $this->input->post('description'),
            );
           
                $this->incomehead_model->mess_incomehead( $data);
          
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Income head added successfully</div>');
            redirect('admin/mess_income/mess_income_head');
        }

        $income_result = $this->incomehead_model->get_headlist();
        $data['categorylist'] = $income_result;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/incomehead/mess_incomehead', $data);
        $this->load->view('layout/footer', $data);
    }

	
	
	function edit_messhead($id) {
        if (!$this->rbac->hasPrivilege('income', 'can_edit')) {
            access_denied();
        }
        $data['title'] = 'Edit Fees Master';
        ...
        $income_result = $this->incomehead_model->get_headlist();
        $data['categorylist'] = $income_result;
        $income_result = $this->incomehead_model->get_headlist($id);
        $data['edit'] = $income_result;
       
         $this->form_validation->set_rules('incomehead', 'Income head', 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/incomehead/mess_incomeheadEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id' => $id,
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Income head updated successfully</div>');
            redirect('admin/mess_income/mess_income_head');
        }
    }
	
	
	function delete_messhead($id) {
        if (!$this->rbac->hasPrivilege('income', 'can_delete')) {
            access_denied();
        }
        $data['title'] = 'Fees Master List';
        $this->incomehead_model->remove_messhead($id);
        redirect('admin/mess_income/mess_income_head');
    }
