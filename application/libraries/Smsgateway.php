...
class Smsgateway {
    ...
    function sendSMS($send_to, $msg) {
        ...
        $fullApi='http://prioritysms.a4add.com/api/sendhttp.php?authkey=370542AOr2vJUUIBxO61b1fe5cP1&mobiles={num}&message={msg}&sender=PcdsIT&route=4&country=91&unicode=1&DLT_TE_ID={tid}';
		$tid='1207167402999202081';
...
function sendSMSdynamic($send_to, $msg,$tid) {
    ...
    function sentAbsentStudentSMS($detail) {
        ...
        if (!empty($sms_detail)) 
		{
            ...
            if ($sms_detail->type == 'clickatell') 
			{
                $params = array('apiToken' => $sms_detail->api_id);
                $this->_CI->load->library('clickatell', $params);
                try 
				{
                    $result = $this->_CI->clickatell->sendMessage(['to' => [$send_to], 'content' => $msg]);
			...
			{
                $params = array('authkey' => $sms_detail->authkey,
                    			'senderid' => $sms_detail->senderid);
			...
			{
                $params = array('username' => $sms_detail->username,
                    			'sernderid' => $sms_detail->senderid,
                    			'password' => $sms_detail->password);
                $this->_CI->load->library('smscountry', $params);
                $this->_CI->smscountry->sendSMS($send_to, $msg);
			...
			else if ($sms_detail->type == 'a4add') 
			{
				$params = array('username' => $sms_detail->username,
                    			'sernderid' => $sms_detail->senderid,
                    			'password' => $sms_detail->password);
						
               	$this->_CI->load->library('newsmslib');
            	$from = $sms_detail->contact;
            	$to = $send_to;
            	$message = $msg;
            	$this->_CI->newsmslib->sendSMS($to, $message,$params);
            }
			else 
			{
