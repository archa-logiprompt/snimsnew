...
class Classes extends Admin_Controller {
    ...
    function index() {
		...
		$this->form_validation->set_rules('degree_award', 'Awarded By', 'trim|required|xss_clean');
        ...
        $centre_id=$admin['centre_id'];
            ...
            $class_array = array(
                'centre_id'=>$centre_id,
                'class' => $this->input->post('class'),
				'awarded_by'=>$this->input->post('degree_award')
            );
			
            $sections = $this->input->post('sections');
            $this->classsection_model->add($class_array, $sections);
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Course added successfully</div>');
            redirect('classes');
        }
