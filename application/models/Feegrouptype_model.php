...
class Feegrouptype_model extends CI_Model {
    ...
    public function add($data) {
	 ...
	 public function edit_messmaster($data) {
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('messfeemasters', $data);
        } else {
	...
	public function getmess($id = null) {
        $this->db->select()->from('messfeemasters');
        $this->db->where('id', $id);
        $query = $this->db->get();
        return $query->row();
    }
	...
	public function remove_messfeemaster($id) {
        $this->db->select()->from('messfeemasters');
        $this->db->where('id', $id);
        $query = $this->db->get();
        $result = $query->row();
        $mess_fee_session_id = $result->mess_fee_session_id;

        $this->db->where('mess_fee_session_id', $mess_fee_session_id);
        $num_rows = $this->db->count_all_results('messfeemasters');
        if ($num_rows == 1) {
            $this->db->where('id', $mess_fee_session_id);
            $this->db->delete('mess_fee_session');
        }
        $this->db->where('id', $id);
        $this->db->delete('messfeemasters');
    }
