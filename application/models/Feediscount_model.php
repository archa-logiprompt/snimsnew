...
class Feediscount_model extends CI_Model {
	...
	*/
	...
	public function getDiscountNotApplied($student_session_id = null) {
      /*  $query = "SELECT fees_discounts.*,student_fees_discounts.id as `student_fees_discount_id`,student_fees_discounts.status,student_fees_discounts.student_session_id,student_fees_discounts.payment_id FROM `student_fees_discounts` INNER JOIN fees_discounts on fees_discounts.id=student_fees_discounts.fees_discount_id ";*/
       
	  $this->db->select('fees_discounts.*')->from('fees_discounts');
		$query=$this->db->get();
		return $query->result();
    }
