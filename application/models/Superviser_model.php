 ...
 * 
 */
class Superviser_model extends CI_model {

    public function superviser_leave_request($id = null) {

        if ($id != null) {
            $this->db->where("superviser.staff_id", $id);
        }

       $query = $this->db->select('staff.name,staff.surname,staff.employee_id,superviser.*,leave_types.type')->join("staff", "staff.id = superviser.staff_id")->join("leave_types", "leave_types.id = superviser.leave_type_id")->join("staff_roles", "staff.id = staff_roles.staff_id")->order_by("superviser.id", "desc")->get("superviser");
      
		return $query->result_array();
		//echo $query;
    }
	 
	 public function superviservi() {
        $query = $this->db->select('staff.name,staff.surname,staff.employee_id,superviser.status as statue,superviser.*,leave_types.type')->join("staff", "staff.id = superviser.staff_id")->join("leave_types", "leave_types.id = superviser.leave_type_id")->get("superviser");
	//echo $query;
        return $query->result_array();
    }
...
echo $query;
    ...
    function addSuperviserLeaveRequest($data) {

        if (isset($data['id'])) {

            $this->db->where("id", $data["id"]);
            $this->db->update("superviser", $data);
        } else {

            $this->db->insert("superviser", $data);
