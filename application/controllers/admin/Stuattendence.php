...
Pushpagiri Dental Sciences';
    ...
    public function classattendencereportbyperiod()
    {
        ...
        $this->form_validation->set_rules('section_id', 'Section', 'trim|required|xss_clean');
        $this->form_validation->set_rules('date', 'From', 'trim|required|xss_clean');
        $this->form_validation->set_rules('subject_id', 'Subject', 'trim|required|xss_clean');
        //$this->form_validation->set_rules('month', 'Month', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($students as $index => &$student) {
                ...
                foreach ($dates as $date) {
                    $student['present'][$i] = $this->db->select('count(attendencetype) as present')->from('student_period_attendance')->where('subject_id', $subject)->where('attendencetype', 0)->where('student_id', $student_id)->where('class_id', $class)->where('section_id', $section)->where('date', date('d/m/Y', strtotime($date)))->get()->row()->present;
                    $student['absent'][$i] = $this->db->select('count(attendencetype) as absent')->from('student_period_attendance')->where('subject_id', $subject)->where('attendencetype', 1)->where('student_id', $student_id)->where('class_id', $class)->where('section_id', $section)->where('date', date('d/m/Y', strtotime($date)))->get()->row()->absent;
    ...
    public function classattendencereportbyday()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $section = $this->input->post('section_id');
            $subject = $this->input->post('subject_id');
            $from = "01/" . ($this->input->post('from'));
            $from_month = explode('/', $this->input->post('from'));
            $to = "31/" . ($this->input->post('to'));
            $to_month = explode('/', $this->input->post('to'));
            $month_difference = abs($from_month[0] - $to_month[0]);
            $monthnum = [];

            for ($i = (int) $from_month[0]; $i <= (int) $to_month[0]; $i++) {
                $monthnum[] = $i;
            }
            ...
            $data['month_selected'] = ($from_month[0]);
            ...
            if ($month_difference == 0) {
                ...
                $data['multiple'] = true;
                $totalmonthAttendance = array();

                $totaldate_result = array();
                $totalattendence_array = array();
                $totalstudent_result = array();
                while ($from_month[0] <= $to_month[0]) {
                    $from = "01/" . $from_month[0] . "/" . $year;
                    $startMonth = $from;
                    $centenary = substr($session_current, 0, 2); //2017-18 to 2017
                    $year_first_substring = substr($session_current, 2, 2); //2017-18 to 2017
                    $year_second_substring = substr($session_current, 5, 2); //2017-18 to 18
                    ...
                    $from_month[0]++;
                    $totalmonthAttendance[] = $monthAttendance;

                    $totaldate_result[] = $date_result;
                    $totalattendence_array[] = $attendence_array;
                    $totalstudent_result[] = $student_result;
                }
                // var_dump($totaldate_result[0]['2024-01-01']);exit;

                $data['monthAttendance'] = $totalmonthAttendance;

                $data['resultlist'] = $totaldate_result;
                $data['attendence_array'] = $totalattendence_array;
                $data['student_array'] = $totalstudent_result;




                // var_dump($monthAttendance);exit;
            }
