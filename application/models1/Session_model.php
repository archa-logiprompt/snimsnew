...
class Session_model extends CI_Model {
    ...
    public function getAllSession() {
        ...
        $sql = "SELECT sessions.*, IFNULL(sch_settings.session_id, 0) as `active`, IFNULL(sch_settings.id, 0) as `sch_setting_id` FROM `sessions` LEFT JOIN sch_settings ON sessions.id=sch_settings.session_id WHERE sessions.centre_id=" . $admin['centre_id'] . " ORDER BY sessions.session ASC";
...
public function remove_financial_year($id)
		...
		public function add_invoice($data)
	{
		...
		{
			$this->db->where('id',$data['id']);
			$this->db->update('starting_invoice',$data);
			
		}
		else{
			
		$this->db->insert('starting_invoice',$data);
