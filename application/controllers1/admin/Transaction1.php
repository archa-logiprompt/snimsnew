            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($studentlist)) {
                    foreach ($studentlist as $key => $eachstudent) {
                        ...
                        $student_total_fees = $this->studentfeemaster_model->getStudentFees($student_id);
...
// print_r($amount_detail);
  ...
  function balancemessfeereport() {
        if (!$this->rbac->hasPrivilege('balance_fees_report', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'mess fee');
        $this->session->set_userdata('sub_menu', 'mess fee/balance fees report');
        $data['title'] = 'student fee';
        $data['title'] = 'student fee';
        $class = $this->class_model->get();
        $data['classlist'] = $class;
      ...
      /*  if ($this->input->server('REQUEST_METHOD') == "GET") {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/transaction/balanceMessfeeReport', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $this->form_validation->set_rules('section_id', 'Section', 'trim|required|xss_clean');
            $this->form_validation->set_rules('class_id', 'Class', 'trim|required|xss_clean');
            if ($this->form_validation->run() == FALSE) {
               ...
               /* $class_id = $this->input->post('class_id');
                ...
                $feetype_arr = $this->input->post('feetype_arr');*/
                $student_Array = array();
                $studentlist = $this->student_model->searchByClassSection();
                ...
                if (!empty($studentlist)) {
                    foreach ($studentlist as $key => $eachstudent) {
                        ...
                        $student_session_id = $eachstudent['student_session_id'];
						$student_id=$eachstudent['id'];
                        $student_total_fees = $this->studentfeemaster_model->getStudentMessFees($student_id);
...
// print_r($amount_detail);
    ...
    function fees_report()
    {
	  ...
	  $class = $this->class_model->get();
      $data['classlist'] = $class;
      $this->session->set_userdata('sub_menu', 'transaction/fees_report');

    if ($this->input->server('REQUEST_METHOD') == "GET") {
	  $this->load->view('layout/header');
      $this->load->view('admin/transaction/feesreport',$data);
	  ...
	  else
	  {
		  ...
		  $date_from = date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date_from')));
          $date_to = date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date_to')));
		  $studentlist = $this->student_model->searchByClassSection($class_id, $section_id,$date_from,$date_to);
		  ...
		  $data['student_due_fee'] = $studentlist;
		  $data['class_id'] = $class_id;
          $data['section_id'] = $section_id;
		  $this->load->view('layout/header');
         $this->load->view('admin/transaction/feesreport',$data);
